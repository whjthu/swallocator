...
class BlockAllocator {
  public:
    ...
    struct FreeNodeInfo {
        size_t addr;
        size_t blockSize;
        FreeNodeInfo(size_t _addr, size_t _blockSize)
            : addr(_addr), blockSize(_blockSize) {}
    };
  ...
  private:
    ...
    // std::set<FreeNodeInfo, CMPFreeNodeInfo, PoolAllocator<FreeNodeInfo> >
    // m_freeNodeSet;
  ...
  public:
    BlockAllocator(const size_t totalSize = MAX_MEM_SIZE,
                   const PlacementPolicy pPolicy = FIND_BEST,
                   const size_t alignment = DEFAULT_ALIGNMENT);

    ~BlockAllocator();

    size_t GetMemUsed() const { return m_used; }

#ifdef DEBUG_ALLOCATOR
    size_t GetMemAsked() const { return m_asked; }
#endif
  ...
  private:
    ...
    // Node *FindBest(const size_t size, const size_t alignment, size_t
    // &padding);

    Node *FindBestInSet(const size_t size, const size_t alignment,
                        uint32_t &padding);

    // void FindFirst(const size_t size, const size_t aligment, size_t &padding,
    ...
    uint32_t CalculatePaddingWithHeader(const size_t baseAddr,
